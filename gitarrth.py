...
class SaurabhV18:
    ...
    def menu(self):
        os.system(f"cls & title SaurabhV18 Nuker ^| Authenticated as: {__client__.user.name}#{__client__.user.discriminator}")
        print(SaurbhV18_art + options + "\n")
        ...
        if ans in ["1", "01"]:
            ...
            try:
                response = self.session.get("https://github.com/GokuNuker/SaurbhV18/releases/latest")
                check_version = response.headers.get('location').split('/')[7].split('v')[1]
                if VERSION != check_version:
                    print("{}({}SaurbhV18{}) You're using an outdated version!".format("\x1b[0m", "\x1b[38;5;21m", "\x1b[0m"))
                    webbrowser.open(f"https://github.com/notspeezy/SaurbhV18-Nuker/releases/tag/{check_version}")
                else:
            ...
            print("- SaurabhV18 Nuker on top!\n- Press enter to return.")
            input("")
            self.menu()
        
        elif ans == "12":
            print("{}({}SaurabhV18{}) Thanks for using SaurbhV18 Nuker!".format("\x1b[0m", "\x1b[38;5;21m", "\x1b[0m"))
            time.sleep(1.5)
            os._exit(0)
...
if __name__ == "__main__":
    try:
        os.system("title SaurabhV18 Nuker ^| Authentication & mode con: cols=95 lines=25")
